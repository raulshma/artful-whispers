...
export function FloatingTabBar({
  ...
  const indicatorRenderWidth =
    effectiveTabSlotWidth - INDICATOR_INTERNAL_PADDING;
  ...
  const indicatorX = useDerivedValue(() => {
    ...
    const activeSlotCenterInContentArea =
      (indicatorPosition.value + 0.5) * effectiveTabSlotWidth;
    // Adjust for the tab bar's own content padding to get the absolute center
    const absoluteActiveSlotCenter =
      CONTENT_PADDING_HORIZONTAL + activeSlotCenterInContentArea;
  ...
  useEffect(() => {
    ...
    activeIndex,
    totalTabs,
    effectiveTabSlotWidth,
    indicatorRenderWidth,
    animatedScale,
    indicatorPosition,
  ]); // Added animatedScale and indicatorPosition
        ...
        <Canvas style={StyleSheet.absoluteFill}>
          <Group>
            ...
            >
              ...
              />
              <Shadow dx={0} dy={8} blur={24} color={shadowColor} />
              <Shadow dx={0} dy={2} blur={8} color={shadowColor} />
            </RoundedRect>
