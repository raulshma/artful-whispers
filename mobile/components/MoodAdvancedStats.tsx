...
export default function MoodAdvancedStats({ 
    ...
    >
      <View style={styles.moodCardHeader}>
        ...
        <View style={styles.moodCardInfo}>
          ...
          <Text style={[styles.moodCardPercentage, { color: theme.colors.textSecondary }]}>
            {stat.percentage.toFixed(1)}%
          </Text>
        </View>
      </View>      <Text style={[styles.moodCardName, { color: theme.colors.text }]}>
        ...
        <View style={styles.trendCard}>
          ...
          {mostFrequent && (
            <View style={styles.dominantMoodContainer}>
              <Ionicons name={mostFrequent.icon as any} size={40} color={mostFrequent.color} />
              <View style={styles.dominantMoodInfo}>
                <Text style={[styles.dominantMoodName, { color: theme.colors.text }]}>
                  {mostFrequent.mood.charAt(0).toUpperCase() + mostFrequent.mood.slice(1)}
                </Text>
                <Text style={[styles.dominantMoodStats, { color: theme.colors.textSecondary }]}>
                  {mostFrequent.count} check-ins ({mostFrequent.percentage.toFixed(1)}%)
                </Text>
              </View>
            </View>
          )}
  ...
  const renderInsights = () => {
    ...
    if (totalCheckIns === 0) {
      ...
      insightText = 'Your mood patterns show some consistency. Try to identify what triggers different moods.';
    } else {
      ...
      <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.summaryGrid}>
            ...
            {stats.length > 0 && renderDetailCard(
              'Most Frequent',
